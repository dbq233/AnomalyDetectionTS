...
from readNormaTXT import get_label, prepocessSubsequence, getDiscordList, preprocessing
...
def k_IDK(X,k, psi, cycle):
    ...
    return test_scores
# def kNN_brute(data,cycle,k):
#     k=min(k,(int)(len(data)/cycle)-1)
#     knn = KNN(n_neighbors=k)
#     all_data=getDiscordList_cycle(data.reshape(-1),cycle)
#     knn.fit(all_data)
#     return  knn.decision_function(all_data)
